...
if bankfile then
	print("here"..bankstring)
else
	print("notfound")
end
local entry_text = display.newText("3 dollar entry fee", 680, 60, native.systemFont,40)
local banktext = display.newText("Bank: $"..bank,750,20,native.systemFont, 40)
banktext:setTextColor(0,255,0)
local endgame = display.newText("Press cmd R to restart",200,550,native.systemFont,30)
		...
		function cubetres:touch (event)
			...
			if event.phase == "began" then
				if over8 == false then
					...
					file:write("\n"..bank.."+"..winval.."+"..entry_fee.."="..bank+winval+entry_fee)
